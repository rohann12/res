...
@section('subheading', 'All projects')
...
<div>
    ...
    @if ($photo->is_cover)
    <div>Cover Photo</div>
    <div><img src="{{ asset('storage/' . $photo->photo_path) }}" alt="cover" height="" width="200px"></div>
    @else
    ...
    @endif
    @endforeach
    <a href={{ route('project.edit', ['project'=> $project]) }}><button>Edit</button></a>
...
<div class="flex h-20 items-center justify-between ">
    <div class="relative inline-block text-left ml-5">
        <select onchange="window.location.href = this.value"
            ...
            <option value="{{ route('project.index') }}" @if(!$status) selected @endif>All</option>
            <option value="{{ route('project.filterByStatus', ['status' => 'upcoming']) }}" @if($status=='upcoming' )
                selected @endif>Upcoming</option>
            <option value="{{ route('project.filterByStatus', ['status' => 'running']) }}" @if($status=='running' )
                selected @endif>Running</option>
            <option value="{{ route('project.filterByStatus', ['status' => 'completed']) }}" @if($status=='completed' )
        ...
        <tbody>
            ...
            <tr class="bg-white border-b  hover:bg-gray-50 ">
                ...
                <td class="px-6 py-4">
                    ...
                    <span
                        class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full  border border-green-400">
                        Completed</span>
                    @endif
                </td>
