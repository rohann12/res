...
@section('subheading', 'All news and updates')
...
<div>
    ...
    @if ($photo->is_cover)
    <div>Cover Photo</div>
    <div><img src="{{ asset('storage/' . $photo->photo_path) }}" alt="cover" height="200px" width="400px"></div>
    @else
    ...
    @endif
    @endforeach
    <a href={{ route('news.edit', ['news'=> $item]) }}><button>Edit</button></a>
...
<a href="{{ route('news.create') }}"><button>Add new news</button></a> --}}

@section('button')
<!-- Modal toggle -->
<div class="flex h-20 items-center justify-between h-20">
    <div class="relative inline-block text-left ml-5">
        <select onchange="window.location.href = this.value"
            ...
            <option value="{{ route('news.index') }}" @if(!$type) selected @endif>All Types</option>
            <option value="{{ route('news.filterByType', ['type' => 'news']) }}" @if($type=='news' )
                selected @endif>News</option>
            <option value="{{ route('news.filterByType', ['type' => 'article']) }}" @if($type=='article' )
                selected @endif>Articles</option>
            <option value="{{ route('news.filterByType', ['type' => 'blog']) }}" @if($type=='blog' )
                selected @endif>Blogs</option>
                <option value="{{ route('news.filterByType', ['type' => 'career']) }}" @if($type=='career' )
                    selected @endif>Careers</option>
        </select>
    </div>
    <div class="px-3">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b  hover:bg-gray-50 ">
                ...
                <td class="flex px-6 pt-4">
                    <a href="{{ route('news.edit', ['news' => $item]) }}" class="h-full pb-3 px-2 hover:text-red-500 font-medium text-2xl font-medium mb-2 text-gray-900 relative">
